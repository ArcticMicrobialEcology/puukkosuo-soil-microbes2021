...
# a function to perform and plot principal component analysis (PCA)
plotPCA <- function(data_for_pca, metadata, colors, legend_graz_loc="topleft", legend_veg_loc="topright", legend_treat_loc="bottomleft"){ # color for grazed first, then color for ungrazed
  ...
  # perform pca
  pca_data <- prcomp(t(data_for_pca), center = TRUE, scale. = FALSE)
  scores <- as.data.frame(pca_data$x[, 1:2, drop = FALSE])
  ...
  xlim_pca <- range(scores$PC1)
  ylim_pca <- range(scores$PC2)
...
# log2 transform the data for PCA
...
{
  pdf(file = "kegg_MG_PCA.pdf", width = 10, height = 10)
  plotPCA(data_for_pca = data_for_pca, metadata = metadata, colors = c("tan1", "forestgreen"), legend_graz_loc = "topright", legend_veg_loc = "bottomright")
  dev.off()
}
...
# use the clr transformed non-aggregated data for PCA
data_for_pca <- otu_table_clr_filt

# perform and plot PCA
{
  pdf(file = "tax_MG_PCA.pdf", width = 10, height = 10)
  plotPCA(data_for_pca = data_for_pca, metadata = metadata, colors = c("tan1", "forestgreen"), legend_graz_loc = "topright", legend_veg_loc = "bottomright")
  dev.off()
}
...
# perform and plot PCA
{
  pdf(file = "kegg_MT_PCA.pdf", width = 10, height = 10)
  plotPCA(data_for_pca = data_for_pca, metadata = metadata, colors = c("tan1", "forestgreen"), legend_graz_loc = "topright", legend_veg_loc = "bottomright", legend_treat_loc = "topleft")
  dev.off()
}
...
# perform and plot PCA
{
  pdf(file = "tax_MT_PCA.pdf", width = 10, height = 10)
  plotPCA(data_for_pca = data_for_pca, metadata = metadata, colors = c("tan1", "forestgreen"), legend_graz_loc = "bottomleft", legend_veg_loc = "bottomright")
  dev.off()
}
