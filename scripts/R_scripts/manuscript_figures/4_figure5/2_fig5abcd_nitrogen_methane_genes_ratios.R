...
for(dat in 1:length(datas_list)){
  ...
  setwd(paste(project_root, "/downstream/lmm_analysis", sep = ""))
  ...
  {
    pdf(file = paste(names(datas_list)[dat], "_grazing_tpm_values_boxplot_all_one_page_nitrogen.pdf", sep = ""), width = 11.7, height = 8.3, onefile = T)
  ...
  # several in one page
  {
    pdf(file = paste(names(datas_list)[dat],"_grazing_tpm_values_boxplot_all_one_page_methane.pdf"), width = 11.7, height = 8.3, onefile = T)
    par(mar=c(10.1, 4.1, 4.1, 2.1))
    par(mfrow=c(2,5))
  ...
  # plot methane and nitrogen ratios with veg_clusters separated in grazing
  {
    pdf(file = paste(names(datas_list)[dat],"_grazing_nitrogen_methane_ratios_veg_clusters_separate.pdf", sep = ""), width = 11.7, height = 8.3, onefile = T)
    par(mar=c(10.1, 4.1, 4.1, 2.1))
    par(mfrow=c(2,5))
    ...
    if(length(meth_ratio)>1){
      ...
      boxplot(vals~g, col=c("forestgreen","tan3","green3","tan2", "tan1"), las=2, xaxt="n", xlab="", ylim = r_boxplot, outline = F, ylab = "pmoA / mcrA", main = main_text, cex.main = 1.5, cex.axis = 1.4, cex.lab = 1.5, frame = FALSE)
    ...
    if(length(nit_ratio2)>1){
      ...
      g <- meta_use$grazing_vegetation
      boxplot(vals~g, col=c("forestgreen","tan3","green3","tan2", "tan1"), las=2, xaxt="n", xlab = "", ylim = r_boxplot, outline = F, ylab = "(nirK+nirS) / nosZ", main = main_text, cex.main = 1.5, cex.axis = 1.4, cex.lab = 1.5,  frame = FALSE)
      axis(side = 1, at = c(1,2,3,4,5), labels = c("Ungrazed downslope T.Ces", "Grazed upslope T.Ces", "Ungrazed downslope C.Cho", "Grazed upslope C.cho", "Grazed upslope C.Ros"), las=2)
  ...
  plot_names <- c("nirS","nirK","norB","nosZ","nifH","napA","nrfA","narG")
  {
    pdf(file = paste(names(datas_list)[dat],"_snow_treatment_tpm_values_boxplot_all_one_page_nitrogen.pdf"), width = 11.7, height = 8.3, onefile = T)
    par(mar=c(10.1, 4.1, 4.1, 2.1))
    par(mfrow=c(2,5))
    for(ro in 1:nrow(nit_data)){
      vals <- as.numeric(nit_data[ro,])
      ...
      boxplot(vals~g, col=c("gainsboro", "firebrick3", "dodgerblue2"), xlab = "", ylim = r_boxplot, outline = F, ylab = "Copies per million", main = main_text, cex.main = 1.5, cex.axis = 1.4, cex.lab = 1.5,  frame = FALSE)
      ...
      # add sig values
      sig_values_text1 <- paste("Snow decrease P-value:",round(sig_values_lmm_simple[rownames(nit_data)[ro],3],3))
      sig_values_text2 <- paste("Snow increase P-value:",round(sig_values_lmm_simple[rownames(nit_data)[ro],2],3))
      
      mtext(sig_values_text1, line = 0.7, cex = 0.7)
      mtext(sig_values_text2, line = -0.1, cex = 0.7)
    }
    
    # ratios
    if(length(nit_ratio1)>1){
      nit_ratio1 <- nit_ratio1[colnames(data)]
      vals <- as.numeric(nit_ratio1)
      r_boxplot <- range(vals, na.rm = T)
      main_text <- "nrfA - (nirK+nirS)"
      # italize
      ...
      g <- as.factor(meta_use$treatment)
      boxplot(vals~g, col=c("gainsboro", "firebrick3", "dodgerblue2"), xlab = "", ylim = r_boxplot, outline = F, ylab = "nrfA - (nirK+nirS)", main = main_text, cex.main = 1.5, cex.axis = 1.4, cex.lab = 1.5,  frame = FALSE)
      ...
      # add sig values
      sig_values_text1 <- paste("Snow decrease P-value:",round(sig_val_temp$coefficients[4,5],3))
      sig_values_text2 <- paste("Snow increase P-value:",round(sig_val_temp$coefficients[3,5],3))
    ...
    if(length(nit_ratio2)>1){
      nit_ratio2 <- nit_ratio2[colnames(data)]
      vals <- as.numeric(nit_ratio2)
      r_boxplot <- range(vals, na.rm = T)
      main_text <- "(nirK+nirS) / nosZ"
      # italize
      ...
      g <- as.factor(meta_use$treatment)
      boxplot(vals~g, col=c("gainsboro", "firebrick3", "dodgerblue2"), xlab = "", ylim = r_boxplot, outline = F, ylab = "(nirK+nirS) / nosZ", main = main_text, cex.main = 1.5, cex.axis = 1.4, cex.lab = 1.5,  frame = FALSE)
      for(j in 1:length(unique(g))){
        points(x = jitter(rep(as.numeric(unique(g)[j]),length(which(g%in%unique(g)[j]))), factor = 2), y=vals[which(g%in%unique(g)[j])], pch=19, cex=0.7)
      }
  ...
  # several in one page
  {
    pdf(file = paste(names(datas_list)[dat],"_snow_treatment_tpm_values_boxplot_all_one_page_methane.pdf"), width = 11.7, height = 8.3, onefile = T)
    par(mar=c(10.1, 4.1, 4.1, 2.1))
    par(mfrow=c(2,5))
    for(ro in 1:nrow(meth_data)){
      vals <- as.numeric(meth_data[ro,])
      ...
      # add sig values
      sig_values_text1 <- paste("Snow decrease P-value:",round(sig_values_lmm_simple[rownames(meth_data)[ro],3],3))
      sig_values_text2 <- paste("Snow increase P-value:",round(sig_values_lmm_simple[rownames(meth_data)[ro],2],3))
    ...
    # ratios
    if(length(meth_ratio)>1){
      meth_ratio <- meth_ratio[colnames(data)]
      vals <- as.numeric(meth_ratio)
      r_boxplot <- range(vals, na.rm = T)
      main_text <- "pmoA / mcrA"
      # italize
      ...
      g <- as.factor(meta_use$treatment)
      boxplot(vals~g, col=c("gainsboro", "firebrick3", "dodgerblue2"), xlab = "", ylim = r_boxplot, outline = F, ylab = "pmoA / mcrA", main = main_text, cex.main = 1.5, cex.axis = 1.4, cex.lab = 1.5,  frame = FALSE)
  ...
  save(mod_nit_ratio1_simple, mod_nit_ratio1_complex, model_nit_ratio1, 
       ...
       mod_meth_ratio_simple, mod_meth_ratio_complex, model_meth_ratio, ses_info, file = paste(names(datas_list)[dat], "_LMM_models_ratios.RData", sep = ""))
  
  # also print out information on the ratios to help in writing
  summary_ratios <- cbind(summary(meth_ratio), summary(nit_ratio1), summary(nit_ratio2))
  colnames(summary_ratios) <- c("PmoA/McrA", "NrfA-(NirK+NirS)", "(NirK+NirS)/NosZ")
  write.csv(x = summary_ratios, file = paste(names(datas_list)[dat],"_summary_ratios.csv", sep = ""), sep = "\t", quote = F, col.names = T, row.names = T)
