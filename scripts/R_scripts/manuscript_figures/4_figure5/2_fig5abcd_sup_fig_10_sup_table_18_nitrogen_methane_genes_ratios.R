...
boxplotWithoutOutliersVegSep <- function(vals, g, cols, main_text, ylab){
  boxplot(vals~g, col=cols, 
          xaxt="n", 
          xlab="", 
          outline = FALSE, 
          ylab = ylab, 
  ...
  axis(side = 1, at = c(1,2,3,4,5), 
       labels = c("Ungrazed downslope T.Ces", "Grazed upslope T.Ces", "Ungrazed downslope C.Cho", "Grazed upslope C.cho", "Grazed upslope C.Ros"), 
       las=2)
...
for(dat in 1:length(datas_list)){
  ...
  # plot methane and nitrogen ratios with veg_clusters separated in grazing
  {
    ...
    if(length(meth_ratio)>1){
      ...
      cols <- c("forestgreen","tan3","green3","tan2", "tan1")
      ylab <- main_text
      boxplotWithoutOutliersVegSep(vals = vals, g = g, main_text = main_text, cols = cols, ylab = ylab)
  ...
  plot_names <- c("nirS","nirK","norB","nosZ","nifH","napA","nrfA","narG")
  {
    ...
    for(ro in 1:nrow(nit_data)){
      ...
      cols <- c("gainsboro", "firebrick3", "dodgerblue2")
      ylab <- "Copies per million"
      boxplotWithoutOutliers(vals = vals, g = g, main_text = main_text, cols = cols, ylab = ylab)
