...
for(dat in 1:length(datas_list)){
  ...
  
  {
    ...
    for(ro in 1:nrow(sig_cors)){
      ...
      p_val <- round(sig_cors$pvalue[ro],3)
      if(p_val==0){p_val <- "<0.001"}
      
      fdr_val <- round(sig_cors$fdr[ro],3)
      if(fdr_val==0){fdr_val <- "<0.001"}

      mtext(paste("P-value:", p_val , "False discovery rate:", fdr_val), cex = 0.8, line = -0.1)
    }
    dev.off()
  }
